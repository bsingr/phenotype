...
class PhenotypeBackend_Editor_Content_Standard extends PhenotypeBackend_Editor
{
	...
	function execute($scope,$action)
	{
		...
		$this->setPageTitle("Phenotype ".$myPT->version. " Content");
	...
	function renderSearch()
	{
		...
		if ($this->con_id!=0)
		{
			...
			$rs = $myDB->query($sql_con);
			$row = mysql_fetch_array($rs);
			$headline="Suche Content / " . $row["con_bez"];
	...
	function renderEdit()
	{
		...
		{
			$myPT->startBuffer();
			if ($myCO->row["dat_status"]=="1") {echo"Online. ";}else{echo"Offline. ";}
			$myAdm->displayCreationStatus($myCO->row["usr_id_creator"],$myCO->row["dat_creationdate"]);
			echo "<br>";
			$myAdm->displayChangeStatus($myCO->row["usr_id"],$myCO->row["dat_date"]);
			$html = $myPT->stopBuffer() . "<br><br>";
	...
	function copy()
	{
		...
		$myCO_Target->set("bez","Kopie von " . $myCO_Source->get("bez"));
		...
		$_params["b"]=0;
		$_params["c"]="akt";
		$_params["feedback"] ="Datensatz kopiert.";
...
<table width="199" border="0" cellpadding="0" cellspacing="0">
  <tr>
    <td class="windowBlank"><table width="100%" border="0" cellpadding="0" cellspacing="3">
        <tr>
          <td class="tableKalenderTage">MO</td>
          <td class="tableKalenderTage">DI</td>
          <td class="tableKalenderTage">MI</td>
          <td class="tableKalenderTage">DO</td>
          <td class="tableKalenderTage">FR</td>
