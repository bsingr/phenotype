...
if ($block_nr==0)
{
  ...
  $mySQL->addField("pag_alttitel",$myRequest->get("alttitel"));
  ...
  if ($myPT->getPref("edit_pages.show_pageurl")==1)
  {
    ...
    if ($row["pag_props"]!="")
    {
      $_props = unserialize($row["pag_props"]);
    }
    ...
    if ($multilanguage==1)
    {
      foreach ($PTC_LANGUAGES AS $k =>$v)
      {
        $_props["pag_url".$k]=$myRequest->get("pag_url".$k);
      }
    }
    else
    {
      $_props["pag_url1"]=$myRequest->get("pag_url1");
    }
    $mySQL = new SqlBuilder();
