...
if (!isset($_REQUEST["editbuffer"]) OR $languagechange == 1)
{
  ...
  // Pr�fung, ob es in der Zielsprache Bausteine gibt
  ...
  if ($row["C"]==0) // von Sprache 1 kopieren
  {
    ...
    {
      if ($block_nr>0 AND $block_nr<77) // Kopierhinweis nur bei Bausteinbl�cken und wenn nicht die Standardsprache gew�hlt wurde
      {
        $language_copy=1;
      }
    }
...
<body>
  ...
  {
    ?>
    previewPage(<?php echo $_REQUEST["id"] ?>,<?php echo $_REQUEST["ver_id"] ?>,<?php echo $_SESSION["lng_id"] ?>,<?php echo $myPT->getPref("preview_dialog.dialog_width",800) ?>,<?php echo $myPT->getPref("preview_dialog.dialog_heigth",500) ?>, "<?php echo localeH("Preview");?>");
    <?php
  }
  ?>
  </script>
...
// Initalisierung des fuer alle Tabs gleichermassen gueltigen Formulars
    ...
    if ($block_nr==0 AND $mySUser->checkRight("elm_pageconfig"))
    {
      ...
      $html = $myLayout->workarea_form_text(locale("Title"),"titel",stripslashes($myPage->titel)); // Changed by Dominique B�s - 2007/08/19
      ...
      // Bestimmen welche Layouts genutzt werden d�rfen
      ...
      // Jetzt ausgew�hlte Seitenvariablen
      ...
      //Get the cache times from the preferences XML-file | added 07/08/23 by Dominique B�s
     ...
     $html = $myLayout->workarea_form_text(locale("Page name"),"bez",stripslashes($myPage->bez)); // Changed by Dominique B�s - 2007/08/19
     ...
     $html.=   $myLayout->workarea_form_textarea(locale("Comment"),"comment",stripslashes($myPage->row["pag_comment"])); // Changed by Dominique B�s - 2007/08/19
