...
if (($_POST['sql_query'] || $_FILES['sql_file']) && $_POST['db']) {
    ...
    if (is_uploaded_file($_FILES['sql_file']['tmp_name'])) {
        if (preg_match("'\.sql\.gz$'",$_FILES['sql_file']['name'])) {
            $sql_file_lines=PMBP_ungzip("lines",$_FILES['sql_file']['tmp_name']);
        } elseif (preg_match("'\.sql\.zip$'",$_FILES['sql_file']['name'])) {
            $sql_file_lines=PMBP_unzip("lines",$_FILES['sql_file']['tmp_name'],$_FILES['sql_file']['name']);
        } else {
            $sql_file_lines=file($_FILES['sql_file']['tmp_name']);
        }
        
        // trim lines and remove comments
        $sql_file="";
        foreach($sql_file_lines as $line) {
            if (trim($line) && substr(trim($line),0,1)!="#" && substr(trim($line),0,2)!="--") $sql_file.=trim($line)."\n";
        }
        
        // do all below once for the POST-data and once for the file
...
echo "<br><br>\n<input type=\"submit\" value=\"".SQ_SEND."\" class=\"button\"><br>\n";
echo "</div>\n</form>\n";
PMBP_print_footer();
?>
